...
from athena_core.domain.portfolio.portfolio_plugins import register_builtin_portfolio_plugins
from athena_core.domain.ml.ml_plugins import register_builtin_ml_plugins
from athena_core.domain.research.research_plugins import register_builtin_research_plugins
from athena_core.domain.statistics.statistics_plugins import register_builtin_statistics_plugins
...
def bootstrap_athena_core(config: AthenaConfig, *, wire_data: bool = True) -> CoreContext:
    ...
    register_builtin_portfolio_plugins(plugin_registry)
    register_builtin_statistics_plugins(plugin_registry)
    register_builtin_research_plugins(plugin_registry)
    register_builtin_ml_plugins(plugin_registry)
    event_bus = EventBus()
